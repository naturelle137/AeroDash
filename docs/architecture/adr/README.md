...
*   **`0xx-[Name]` to `2xx-[Name]` (e.g., `001-notification-system`)**: **Product Architecture**. These ADRs govern the core logic, features, data schemas, functional architecture, and anything directly affecting the runtime application. The number space from 001 to 299 is reserved for these.
*   **`3xx-DEV-[Name]` and `4xx-DEV-[Name]` (e.g., `300-DEV-documentation-as-code`)**: **Development Architecture**. These ADRs govern the developer experience (DX), tooling, CI/CD processes, branching strategies, code formatting rules, and everything related to *how* we build the software.

## ADR Lifecycle

1.  **Drafting**: A contributor creates a new ADR file starting with the next available number in the appropriate series (0xx-2xx or 3xx-4xx), using the [`000-template.md`](000-template.md) as a blueprint. The status is `Proposed`.
