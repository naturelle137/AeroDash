...
| **Pending requirements** | A `@REQ-` tag has no downstream IMP or DES link. The CI workflow `jq` query treats every `@REQ-` equally and does not consult the REQ `Status:` field, so `Deferred` / `Deprecated` REQs **still surface in the CI log** as pending. Status-aware exclusion of `Deferred` (out of scope for the current release cycle) and `Deprecated` (withdrawn) is implemented only in the local `pnpm trace check` CLI (`reqCoverage.excludedReqs`) — see [`docs/requirements/README.md` § Status Lifecycle](../requirements/README.md) and the audit forensics emitted by `runCheck`. |
...
| **Unverified P1 requirements** | An implemented `@REQ-` has no `@E2E-` anywhere in its chain. The CI workflow's `jq` query, like the pending-requirements row above, does not read the REQ `Status:` field — `Deferred` / `Deprecated` REQs still surface in the CI log. Status-aware exclusion lives only in `pnpm trace check` — see [`docs/requirements/README.md` § Status Lifecycle](../requirements/README.md). |
