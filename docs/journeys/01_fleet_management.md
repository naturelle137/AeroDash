...
<!-- @UJ-A-004@ (FROM: @REQ-AD-020@, @REQ-AD-021@, @REQ-AD-022@, @REQ-UI-021@, @REQ-UI-022@, @REQ-UI-023@, @REQ-FE-006@) -->
...
| **Later Correction**     | A month later opens the saved Velis profile in the editor to tighten the reserve floor to `4.0 kWh`.             | "Just tweaking the reserve — nothing structural."                | The editor shows `Powertrain: Electric` as a read-only badge with no radio group. Selecting combustion `Pipistrel P2008 JC` in the model dropdown does not flip `powertrain`; the stored `electric` is retained and the reserve-floor change saves.  |

**Outcome:** The instructor adds the Velis Electro without encountering a single combustion-only concept. The system represents the aircraft on its own terms: battery pack, reserve floor, energy-based endurance. Combustion profiles behave identically to before — no regression, no mixed UI. The shoehorn defect called out in issue #225 is gone. Once the profile is saved, its propulsion type is frozen: the pilot cannot accidentally flip `Powertrain` through the editor, eliminating the orphaned-topology failure mode called out in issue #231.
