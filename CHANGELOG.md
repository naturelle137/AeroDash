...
- Post-save action chooser in the aircraft wizard: after saving a draft the pilot can jump straight into flight prep (sets the new draft as the active aircraft and routes to Mass & Balance, emitting `WARN-AC-002`), go verify it, or return to the fleet — replacing the unconditional bounce back to the fleet list (UX-012, REQ-AC-001/REQ-AC-005, closes #279)
...
- Wizard post-save **Verify now** now opens the sign-off dialog on arrival at the fleet list (handed off via a `?verify=<id>` query that is consumed and stripped on mount) instead of leaving the new aircraft unverified next to its shield icon (UX-012, REQ-AC-001/REQ-AC-007, refs #279)
- Route navigations now reset the scroll position to the top by default while still restoring `savedPosition` on browser back/forward, so jumping into Mass & Balance from a scrolled wizard no longer drops the pilot mid-page (refs #279)
- Mass & Balance aircraft picker now mirrors the pending draft profile while its WARN-AC-002 acknowledgement is awaiting, so the registration just saved in the wizard appears in the dropdown immediately instead of after the pilot confirms (UX-012, REQ-AC-001/REQ-AC-005, refs #279)
...
- Flight Preparation sticky-strip stack uses `IntersectionObserver` per card; eliminates low-end-iPad flicker at strip boundary (TECH-016 / UX-009, refs #282)
- `fleet.repository.ts` caches the `IDBDatabase` handle module-scoped; preflight no longer reopens IndexedDB 3× before the first weight (TECH-017, refs #282)
- `MigrationPoint` carries `moment` alongside `arm`; moment-mode envelope chart plots the burn-down path without recomputing `arm × mass` downstream (TECH-012, refs #282)
- Unit-conversion constants (`US_GAL_TO_L`, `IMP_GAL_TO_L`, `LB_TO_KG`, `IN_TO_M`, `FT_TO_M`) unified in `unit-conversion-constants.ts`; duplicates in `fuel-mass.ts` and `unit-normalization.ts` removed (TECH-014, REQ-SYS-003, refs #282)
- `Violation` gains `path: readonly (string | number)[]` from `mapZodErrorToViolations`; M&B store replaces legacy `STATIONS[…]` regex with structured path index (TECH-015, refs #282)
- `seedRaw` in fleet-repository / data-rights integration suites accepts `object`; drops 14 `as unknown as Record<string, unknown>` casts (TECH-020, refs #282)
- `oxlint` and `eslint-plugin-oxlint` pinned to exact `1.66.0`; `.oxlintrc.json` promotes `no-await-in-loop`, `no-self-compare`, `no-unreachable-loop` to explicit warn-level rules (TECH-021, refs #282)
- Reset-Payload button gains hover `title` and `aria-describedby` hint clarifying station-weights-only scope (TECH-026, refs #282)
- `deep: true` removed from session-persistence watcher; fresh-array projection already triggers on every relevant change (TECH-027, refs #282)
- Code-comment discipline rule hardened: `CLAUDE.md § Code comments` + `CONTRIBUTING.md §4.x Code Comments` make the why-only / trace-tag-preferred / no-forbidden-identifier rule the project-wide default, and `/implement-issue` carries the same rule inline so future Claude runs cannot drift; mechanical enforcement via a new `comment-discipline` scanner (`frontend/scripts/comment-discipline/`) wrapped in a vitest gate (`repo-comment-discipline-gate.spec.ts`) and the `pnpm --filter frontend comment:check` / `comment:baseline` scripts — baseline-ratcheted so pre-existing matches are grandfathered and only new regressions hard-fail
- On-demand `@claude` mention bot (`.github/workflows/claude.yml`): typing `@claude` in a new issue, an issue/PR comment, or a PR review now spawns `anthropics/claude-code-action@v1` so Claude can answer questions, draft changes, or open PRs from the thread. Uses `secrets.CLAUDE_CODE_OAUTH_TOKEN`, defaults to `sonnet` + `high` effort (matching `pr-review-bot.yml` / `overnight-implementation.yml`), guards on a missing-secret skip step, serialises per issue/PR via a `claude-mention-${{ issue|pr number }}` concurrency group, and whitelists only `claude[bot]` as a permitted initiating bot
- `@claude` mention bot (`.github/workflows/claude.yml`) now passes `allowed_tools: 'Edit(.claude/**),Write(.claude/**),Edit(.github/workflows/**),Write(.github/workflows/**)'` to `claude-code-action@v1` so an @claude sweep can land bot-config tweaks under `.claude/**` (skills/agents/commands) and `.github/workflows/**` directly, instead of failing the write step and forcing a manual branch. Pushing workflow-file changes still requires the underlying GitHub App to hold the `workflows` permission — the tool allow-list does not bypass that scope check
- Overnight implementation bot (`.github/workflows/overnight-implementation.yml`) gains a `max_turns` `workflow_dispatch` input so a one-off manual run can raise or lower `--max-turns` without editing `env.MAX_TURNS`; leaving the field blank falls back to the workflow default
