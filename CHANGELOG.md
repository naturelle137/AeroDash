...
- Stryker mutation testing scoped to `frontend/src/core/**` (P1 Safety Core) with `thresholds.high = 85` / `thresholds.break = 70`, driven by `vitest.config.p1.ts` for framework-free runs; new `pnpm --filter frontend test:mutation` script and TESTING.md §7 document the runner, exclusions, and local workflow. CI gate wired up as `.github/workflows/mutation.yml` (P1 Mutation Score check on PRs to `develop`/`main`, no `continue-on-error`, HTML report uploaded as artifact) — a score below `thresholds.break` now hard-fails the PR (closes #260, TECH-007/PR-015)
