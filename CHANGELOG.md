...
- Stryker mutation testing scoped to `frontend/src/core/**` (P1 Safety Core) with `thresholds.high = 85` / `thresholds.break = 70`, driven by `vitest.config.p1.ts` for framework-free runs; new `pnpm --filter frontend test:mutation` script and TESTING.md §7 document the runner, exclusions, and local workflow. CI gate wiring is owned by `.github/workflows/` and follows separately (refs #260, TECH-007/PR-015)
