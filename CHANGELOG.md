...
- Redesigned fleet-list action buttons for mobile / cockpit use: Edit is now a pencil icon, Delete a trash icon, Download a download-arrow icon. Icon buttons are 44×44 px (≥48×48 px on mobile) with 0.75 rem spacing to meet WCAG 2.2 SC 2.5.8 target-size AAA and remain tappable under moderate vibration. Select / Verify keep their text labels so safety-critical state transitions stay unambiguous. Hover / focus-visible / active states use theme CSS variables for light/dark parity (refs REQ-UQ-003)
