...
- Engineering cleanup batch from the v0.3.0-alpha release audit (closes #282): IntersectionObserver-based sticky-strip stack on the Flight Preparation view replaces the RAF-throttled `scroll`/`resize` listener so per-strip dock/release fires only on threshold crossings, eliminating the low-end-iPad flicker (TECH-016 / UX-009); `fleet.repository.ts` now caches the `IDBDatabase` handle module-scoped and recycles it across CRUD calls, releasing it on `onversionchange` / `onclose` and on the new `_resetFleetDbHandleForTest` hook, so a preflight no longer reopens IndexedDB 3× before the first weight (TECH-017); `MigrationPoint` carries `moment` alongside `arm` so moment-mode envelope charts plot the burn-down path without recomputing `arm × mass` downstream, and `mass-balance.logic.ts` populates it on every burn waypoint (TECH-012); the four duplicate US-gallon / lb / inch / foot literals on `unit-normalization.ts` and `fuel-mass.ts` are unified behind a single `unit-conversion-constants` module so a future precision update cannot silently diverge (TECH-014, REQ-SYS-003); `Violation` now exposes a structured `path: readonly (string | number)[]` populated by `mapZodErrorToViolations` and consumed by the M&B store, replacing the regex on the legacy stringified `field` (TECH-015); typed-fixture refactor of the fleet-repository / data-rights integration suites — `seedRaw` accepts `object` so an `AircraftProfile`-plus-extras can be passed without an `as unknown as Record<string, unknown>` ladder (TECH-020); oxlint and `eslint-plugin-oxlint` pinned to exact `1.66.0` and `.oxlintrc.json` upgraded from category-only (`correctness: error`) to explicit rule allow-list (`no-await-in-loop`, `no-self-compare`, `no-unreachable-loop` warn-level) so a transitive bump cannot silently relax the lint floor (TECH-021); Reset-Payload tooltip + inline hint surface that the action clears station weights only — the aircraft profile, fuel sequences and Verified status are kept (TECH-026); `deep: true` dropped from the session-persistence watcher because the projection already produces a fresh array reference on every reactive tick, with a profiling note explaining why the bounded projection beats the redundant deep traversal on low-end iPads (TECH-027).
