...
**Principle.** Default to the fewest words that convey intent. Every artifact you produce in this repository — code, comments, commit messages, PR descriptions, code reviews, issue bodies, changelog and requirement entries, ADRs, and **any artifact type introduced by a future skill or document type** — is word-efficient by default. Prefer bullets over prose; omit justification, restatement, and design padding unless explicitly asked; say what is needed, then stop. This is a standing default, not a per-artifact opt-in: it governs **all current *and future* skills and document types** automatically, so a new skill or document type inherits it without restating it.
